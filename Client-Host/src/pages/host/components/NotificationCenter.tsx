        ...
        <div className="relative" ref={dropdownRef}>
            <button 
                ...
                className={`relative p-2.5 transition-all rounded-full ${
                    isOpen ? 'bg-orange-100 text-[#E57A44]' : 'text-slate-400 hover:text-[#E57A44] hover:bg-orange-50'
                }`}
            >
                <Bell size={20} />
                {unreadCount > 0 && (
                    <motion.span 
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className="absolute top-2 right-2.5 w-4 h-4 bg-[#E57A44] text-white text-[9px] font-bold flex items-center justify-center rounded-full border-2 border-white"
                    >
                        ...
                        <div className="p-4 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                            ...
                            {unreadCount > 0 && (
                                <button 
                                    ...
                                    className="text-[10px] font-bold text-[#E57A44] hover:underline flex items-center gap-1"
                                >
                                ...
                                <div className="divide-y divide-slate-50">
                                    {notifications.map((n) => (
                                        <div 
                                            ...
                                            className={`p-4 hover:bg-slate-50 transition-colors cursor-pointer relative group ${!n.isRead ? 'bg-orange-50/30' : ''}`}
                                        >
                                            <div className="flex gap-3">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    {n.actionUrl && (
                                                        <div className="mt-2 flex items-center gap-1 text-[9px] font-bold text-[#E57A44] opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            {!n.isRead && (
                                                <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#E57A44]" />
                                            )}
                                        </div>
                                    ))}
                                </div>
